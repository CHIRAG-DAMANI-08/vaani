  ...
  try {
    ...
    if (transporter) {
      ...
      try {
        ...
        logger.info({ messageId: info.messageId, applicationId: id, to: application.email }, "Beta acceptance email sent");
      } catch (error) {
        logger.error({ err: error, applicationId: id, to: application.email }, "Beta acceptance email failed");
      }
    } else {
      logger.warn("SMTP not configured; beta acceptance email not sent");
    }
